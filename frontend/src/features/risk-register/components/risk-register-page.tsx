...
interface RegisterRow {
  ...
  // Pre-computed per-row to avoid repeated filter calls in render
  missingGaps:       number | null;
  partialGaps:       number | null;
  qScore:            number | null;
  contractOk:        boolean;
}
...
function buildRow(
  ...
  // Pre-compute gap counts in one pass rather than filtering per column in render
  let missingGaps: number | null = null;
  let partialGaps: number | null = null;
  if (latestDora?.results?.gaps) {
    missingGaps = 0;
    partialGaps = 0;
    for (const g of latestDora.results.gaps) {
      if (g.gapLevel === 'missing') missingGaps++;
      else if (g.gapLevel === 'partial') partialGaps++;
    }
  }

  let a30Missing = 0;
  if (latestA30?.results?.gaps) {
    for (const g of latestA30.results.gaps) {
      if (g.gapLevel === 'missing') a30Missing++;
    }
  }
  ...
  return {
    ...
    missingGaps,
    partialGaps,
    qScore: latestQ?.status === 'complete' ? (latestQ.overallScore ?? null) : null,
    contractOk: latestA30?.status === 'complete' && a30Missing === 0,
  };
}
...
export function RiskRegisterPage() {
  ...
  const rows: RegisterRow[] = useMemo(() => {
    const ws  = allWorkspaces ?? workspaceList;
    const ass = allAssessments ?? [];
    const qs  = allQuestionnaires ?? [];
    return ws.map((w) => buildRow(w as WorkspaceWithMembership, ass, qs));
  }, [allWorkspaces, workspaceList, allAssessments, allQuestionnaires]);

  const { highRiskCount, completeCount, expiringCount } = useMemo(() => {
    let high = 0, complete = 0, expiring = 0;
    for (const r of rows) {
      if (r.latestDora?.results?.overallRisk === 'High') high++;
      if (r.stepsComplete === 5) complete++;
      if (r.nearestCertDays !== null && r.nearestCertDays <= 90) expiring++;
    }
    return { highRiskCount: high, completeCount: complete, expiringCount: expiring };
  }, [rows]);
      ...
      {isLoading ? (
        ...
        <div className="rounded-md border overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {rows.map(({ workspace, latestDora, latestA30, stepsComplete, nearestCertDays, reviewDays, riskMatrix, missingGaps, partialGaps, qScore, contractOk }) => {
                return (
                  <TableRow
                    key={workspace.id}
                    ...
                    <TableCell>
                      {latestDora ? (
                        <span className="text-xs">
                          <span className="text-destructive font-medium">{missingGaps}</span>
                          <span className="text-muted-foreground"> / </span>
                          <span className="text-amber-600 font-medium">{partialGaps}</span>
                        </span>
                      ) : (
